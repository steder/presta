...
class TestPrestaServer(unittest.TestCase):
    def test_serverName(self):
        self.server = server.PrestaServer(6667)
        webServer = self.server.getServiceNamed("Presta")
        self.assertEqual("Presta", webServer.name
                         )


class TestPrestaServerStart(unittest.TestCase):
    ...
    def setUp(self, mockTwistedApplicationService):
        ...
        portNumber = 6666
        self.server = server.PrestaServer(portNumber)
